class TreatmentsController < ApplicationController
  ...
  def create
    ...
    if @treatment.save
      redirect_to my_patient_path(@patient_record), notice: "Treatment added successfully."
    else
      redirect_to my_patients_path, alert: @treatment.errors.full_messages.join(", ")
    end
  end
  ...
  def update
    if @treatment.update(treatments_params)
      redirect_to my_patient_path(@patient_record), notice: "Treatment updated successfully."
    else
      redirect_to my_patients_path, alert: @treatment.errors.full_messages.join(", ")
    end
  end
  ...
  def destroy
    if @treatment.destroy
      redirect_to my_patient_path(@patient_record), notice: "Treatment destroyed successfully."
    else
      redirect_to my_patients_path, alert: @treatment.errors.full_messages.join(", ")
    end
  end
