class PatientRecordsController < ApplicationController
  before_action :set_patient_record, only: [:show, :assign_to_doctor, :edit, :destroy, :update]
  def index
  ...
  def create
    ...
    if @patient_record.save
      redirect_to patient_records_path, notice: "Patient record created successfully."
    else
      render :new
    end
  end
  ...
  def assign_to_doctor
    ...
    redirect_to my_patients_path, notice: "Patient picked successfully."
  end

  def update
    if @patient_record.update(patient_records_params)
      redirect_to patient_records_path, notice: "Patient record updated successfully."
    else
      render :edit
    end
  end
  ...
  def destroy
    if @patient_record.destroy
      redirect_to patient_records_path, notice: "Patient record destroyed successfully."
    else
      redirect_to patient_records_path, alert: @patient_record.errors.full_messages.join(",")
    end
  end
